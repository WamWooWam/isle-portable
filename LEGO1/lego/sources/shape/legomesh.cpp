...
LegoResult LegoMesh::Read(LegoStorage* p_storage)
{
	...
	if ((result = p_storage->ReadFloat(&m_alpha)) != SUCCESS) {
		return result;
	}
	if ((result = p_storage->ReadU8(&m_shading)) != SUCCESS) {
		return result;
	}
	if ((result = p_storage->ReadU8(&m_unk0x0d)) != SUCCESS) {
		return result;
	}
	if ((result = p_storage->Read(&m_unk0x20, sizeof(undefined))) != SUCCESS) {
		return result;
	}
	if ((result = p_storage->ReadU8(&m_useAlias)) != SUCCESS) {
		return result;
	}

	if ((result = p_storage->ReadU32(&textureLength)) != SUCCESS) {
		return result;
	}
	if (textureLength) {
	...
	if ((result = p_storage->ReadU32(&materialLength)) != SUCCESS) {
		return result;
	}
	if (materialLength) {
