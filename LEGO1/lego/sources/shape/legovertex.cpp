...
{
	LegoResult result;
	if ((result = p_storage->ReadFloat(&m_coordinates[0])) != SUCCESS) {
		return result;
	}
	if ((result = p_storage->ReadFloat(&m_coordinates[1])) != SUCCESS) {
		return result;
	}
	if ((result = p_storage->ReadFloat(&m_coordinates[2])) != SUCCESS) {
		return result;
	}
	return SUCCESS;
}
