...
MxResult LegoAnimPresenter::CreateAnim(MxStreamChunk* p_chunk)
{
	...
	if (storage.ReadS32(&magicSig) != SUCCESS || magicSig != 0x11) {
		goto done;
	}
	if (storage.ReadFloat(&m_unk0xa4) != SUCCESS) {
		goto done;
	}
	if (storage.ReadFloat(&m_unk0xa8[0]) != SUCCESS) {
		goto done;
	}
	if (storage.ReadFloat(&m_unk0xa8[1]) != SUCCESS) {
		goto done;
	}
	if (storage.ReadFloat(&m_unk0xa8[2]) != SUCCESS) {
		goto done;
	}
	if (storage.ReadS32(&parseScene) != SUCCESS) {
		goto done;
	}
	if (storage.ReadS32(&val3) != SUCCESS) {
		goto done;
	}
