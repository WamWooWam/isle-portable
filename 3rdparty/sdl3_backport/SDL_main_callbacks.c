...
#include <SDL2/SDL.h>
#include <SDL3/SDL_sysmain.h>
#include <SDL3/SDL_main_callbacks.h>
...
static SDL_atomic_t apprc;  // use an atomic, since events might land from any thread and we don't want to wrap this all in a mutex. A CAS makes sure we only move from zero once.
...
static void SDL_DispatchMainCallbackEvent(SDL_Event *event)
{
    if (SDL_AtomicGet(&apprc) == SDL_APP_CONTINUE) { // if already quitting, don't send the event to the app.
        SDL_AtomicCAS(&apprc, SDL_APP_CONTINUE, SDL_main_event_callback(SDL_main_appstate, event));
    }
}
...
static void SDL_DispatchMainCallbackEvents(void)
{
    ...
    for (;;) {
        int count = SDL_PeepEvents(events, SDL_arraysize(events), SDL_GETEVENT, SDL_FIRSTEVENT, SDL_LASTEVENT);
        if (count <= 0) {
            break;
        }
...
static int SDLCALL SDL_MainCallbackEventWatcher(void *userdata, SDL_Event *event)
{
    if (ShouldDispatchImmediately(event)) {
        ...
        if (event->type == SDL_APP_TERMINATING) {
            SDL_AtomicCAS(&apprc, SDL_APP_CONTINUE, SDL_APP_SUCCESS);
        }
    } else {
...
SDL_AppResult SDL_InitMainCallbacks(int argc, char *argv[], SDL_AppInit_func appinit, SDL_AppIterate_func appiter, SDL_AppEvent_func appevent, SDL_AppQuit_func appquit)
{
    ...
    if (SDL_AtomicCAS(&apprc, SDL_APP_CONTINUE, rc) && (rc == SDL_APP_CONTINUE)) { // bounce if SDL_AppInit already said abort, otherwise...
        ...
        SDL_AddEventWatch(SDL_MainCallbackEventWatcher, NULL);
...
SDL_AppResult SDL_IterateMainCallbacks(SDL_bool pump_events)
{
    ...
    if (rc == SDL_APP_CONTINUE) {
        ...
        if (!SDL_AtomicCAS(&apprc, SDL_APP_CONTINUE, rc)) {
