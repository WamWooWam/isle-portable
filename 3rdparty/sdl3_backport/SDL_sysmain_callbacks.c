...
static SDL_bool iterate_after_waitevent = SDL_FALSE;
...
static void SDLCALL MainCallbackRateHintChanged(void *userdata, const char *name, const char *oldValue, const char *newValue)
{
    ...
    if (iterate_after_waitevent) {
        ...
        if (callback_rate > 0) {
            callback_rate_increment = ((Uint64) 1000000) / ((Uint64) callback_rate);
        } else {
            callback_rate_increment = 0;
        }
    }
...
int SDL_EnterAppMainCallbacks(int argc, char *argv[], SDL_AppInit_func appinit, SDL_AppIterate_func appiter, SDL_AppEvent_func appevent, SDL_AppQuit_func appquit)
{
    ...
    // printf("SDL_InitMainCallbacks: %d\n", rc);
    if (rc == 0) {
        SDL_AddHintCallback(SDL_HINT_MAIN_CALLBACK_RATE, MainCallbackRateHintChanged, NULL);

        Uint64 next_iteration = callback_rate_increment ? (SDL_GetTicks() + callback_rate_increment) : 0;
